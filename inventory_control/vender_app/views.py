...
def vender_create(request):
    # if this is a POST request we need to process the form data
    if request.method == 'POST':
        print ('post')
        # create a form instance and populate it with data from the request:
        form = vender_form(request.POST)
    ...
    return render(request, 'vender_app/vender_form.html', {'form': form})
...
def vender_view(request):
    if request.method =='GET':
        ...
        prev_pagenumber = 1
        next_page_number = 2
        totaldata = vender_master.objects.all().order_by('created_at').count()


        query_results = vender_master.objects.all().order_by('created_at')[:pagedata_ending]
        ...
        return render(request, 'vender_app/vender_view.html', context)


def vender_view_pagination(request,page_number):
    if request.method =='GET':
        ...
        query_results = vender_master.objects.all().order_by('created_at')[pagedata_starting:pagedata_ending]
        showingdata = query_results.count()
