...
def site_create(request):
    ...
    return render(request, 'site_app/site_form.html', {'form': form})
...
def site_view(request):
    if request.method =='GET':
        ...
        prev_pagenumber = 1
        next_page_number = 2
        totaldata = site_master.objects.all().order_by('created_at').count()


        query_results = site_master.objects.all().order_by('created_at')[:pagedata_ending]
        ...
        return render(request, 'site_app/site_view.html', context)


def site_view_pagination(request,page_number):
    if request.method =='GET':
        ...
        query_results = site_master.objects.all().order_by('created_at')[pagedata_starting:pagedata_ending]
        showingdata = query_results.count()
